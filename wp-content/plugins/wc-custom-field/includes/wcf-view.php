...
function wcf_show_custom_field() {
  ...
  if (get_option('wcf_menu_bg_color')) {
    $wcf_menu_bg_color = get_option('wcf_menu_bg_color');
  }
  if (get_option('wcf_menu_hover_color')) {
    $wcf_menu_hover_color = get_option('wcf_menu_hover_color');
  }
  if (get_option('wcf_menu_text_color')) {
    $wcf_menu_text_color = get_option('wcf_menu_text_color');
  }
  if (get_option('wcf_menu_text_hover_color')) {
    $wcf_menu_text_hover_color = get_option('wcf_menu_text_hover_color');
  }
  if (get_option('wcf_submenu_bg_color')) {
    $wcf_submenu_bg_color = get_option('wcf_submenu_bg_color');
  }
  if (get_option('wcf_prod_name_color')) {
    $wcf_prod_name_color = get_option('wcf_prod_name_color');
  }
  ?>
  <style>
  ...
  <script>
    var img_url_plus = '<?php echo plugins_url(); ?>/wc-custom-field/images/plus.png';
    var img_url_minus = '<?php echo plugins_url(); ?>/wc-custom-field/images/minus.png';
    ddaccordion.init({
...
function wcf_radio($data, $k, $field_lbl) {
  ...
  foreach ($data as $val) {
    ...
    if (count($v) == 2) {
      ...
      echo '<tr>';
      echo "<td class='wcf_item' data-th='Item Name'>$v[0]</td>";
      echo "<td data-th='Item Price'><input id=\"$k\" class=\"$class\" data-cf_type='radio' data-cf_lbl=\"$field_lbl\" data-cf_field=\"$v[0]\" data-cf_value=\"$v[1]\" type='radio' name='cf_radio[]' value='" . $val . "'>&nbsp;" . $price . "</td>";
      echo '</tr>';
      $r++;
    } else {
      return '';
  ...
  if ($r > 0) {
    ...
    echo "<td><input id=\"$k\" class=\"$class\" data-cf_type='radio' data-cf_value='' type='radio' name='cf_radio[]' value='' checked></td>";
    echo '</tr>';
  }
  echo '</table>';
}
...
function wqb_add_custom_price( $cart_object ) 
{
  ...
  {    
    if(isset($value['wcf_custom_data'])){
      if($value['wcf_custom_data']!=''){
      $p=  $value['data']->get_price();
      $rp=  $value['data']->get_regular_price();
      $cp=  $value['data']->get_sale_price();  
      //$cprice=$value['data']->price;  
      $cprice=$value['data']->get_price();
      $cdata=$value['wcf_custom_data'];
      
      
      if (is_array($cdata)) {
        foreach ($cdata as $value2) {
          $cprice=$cprice+$value2;
        }
      }
      //$value['data']->price = $cprice;
      if($p==$rp || $p==$cp){        
        $value['data']->set_price($cprice);
      }
    }
    }    
  }
