...
function wcp_custom_degin_panel($product){
  ...
  <tr>
    ...
    <td>
      <input type="checkbox" name="wcf_status" id="wcf_status"  <?php if( $meta_box_check == true ) { ?>checked="checked"<?php } ?> />
    </td>
  </tr>
  </table>  
      ...
      foreach ($frm_data as $f_data){
        ...
        <td data-th="Type">
          <select  class="wpr_mf_ddl" name="wpr_mf_text[<?php echo $cnt;?>][3]">
            <option value="chk_box" <?=$type=='chk_box' ? ' selected="selected"' : '';?>>Checkbox</option>
            <option value="radio" <?=$type=='radio' ? ' selected="selected"' : '';?>>Radio</option>
            <option value="ddl" <?=$type=='ddl' ? ' selected="selected"' : '';?>>DropDownList</option>
          </select>
          <?php echo $r_field;?>
        </td>   
        <?php
...
function wcf_content_save( $post_id,$product) {
  if($product->post_type == 'product'){
    ...
    if (isset($_POST['wpr_mf_text'])) {
        ...
        $fields=rtrim($fields,',');
        $data=json_encode($_POST['wpr_mf_text']);
