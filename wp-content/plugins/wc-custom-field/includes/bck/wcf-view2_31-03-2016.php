...
function wrs_cf_chk($data, $k, $field_lbl, $post_id){  
  ...
  foreach ($data as $val){
    ...
    if(count($v)==2){      
      ...
      echo "<td data-th='Item Price'><input id=\"$k\" class=\"$class\"  data-cf_type='chk_box' data-pid=\"$post_id\" data-cf_lbl=\"$field_lbl\" data-cf_field=\"$v[0]\" data-cf_value=\"$v[1]\" type='checkbox' name='cf_chk[]' value='$val' onclick='wcf_checkbox_val(this)' />&nbsp;$price</td>";
...
function wrs_cf_radio($data, $k, $field_lbl, $post_id){
  ...
  foreach ($data as $val){
    ...
    if(count($v)==2){  
      ...
      echo "<td data-th='Item Price'><input id=\"$k\" class=\"$class\" data-cf_type='radio' data-pid=\"$post_id\" data-cf_lbl=\"$field_lbl\" data-cf_field=\"$v[0]\" data-cf_value=\"$v[1]\" type='radio' name='cf_radio[]' value='".$val."' onclick='wcf_radio_val()'>&nbsp;".$price."</td>";
      echo '</tr>';
      $r++;
    }else{
      return '';
...
function wrs_cf_ddl($data, $k, $field_lbl, $post_id){
  ...
  foreach ($data as $val){
    ...
    if(!empty($v) && $v[0]&& $v[1]){
      $id='cfield_'.$k;
      echo "<option data-id='$k' data-val='$v[1]' data-pid='$post_id' data-price='$price' value='$val'>$v[0]</option>";
      $r++;
    }else{
      return '';
    }
