  ...
  if ($wfm_tag != '') {
    ...
    if ($loop->have_posts()) {
      ...
      foreach ($loop->posts as $val) {
        ...
        //echo '<div class="wrs_rs_box_inner" style="background-image: url(' . $feat_image . ');">';  
          $current_page_url = add_query_arg('rid', $user->user_id, get_permalink());
          $current_page_url = add_query_arg('pid', $val->ID, $current_page_url);
...
$json = file_get_contents("https://maps.google.com/maps/api/geocode/json?address=$address&key=$key&sensor=false&region=$region");
