...
<div class="span4 alertAdd" style="opacity: 1; display: block;">
  <div class="alert alert-info"id="wfm_alert_info" style="display: none;"><?php _e("Added to your cart","wfm-lang");?> </div>
</div>

<?php if(get_option('wfm_display_mini_cart')==1){ ?>
...
<script>
  ...
  function wfm_add_prod(pid,vid){
    ...
    //alert(ajax_url2);
    var ajax_url = '<?php echo WFM_BASE_URL; ?>';
  ...
  var plugin_url='<? echo WFM_BASE_URL; ?>/includes/wfm-popup-data.php';
</script>  
<?php
        ...
        foreach($loop->posts as $val){
          ...
          if($is_cat==0){
            ...
            if ($variation_display == true){
             ...
             ini_set('display_errors','Off');
             if(function_exists('wrm_item_custom_field')){
               $variation=false;
             }             
            if($variation==true && $product->is_type( 'variable' )){
              //----------------------------------------------------              
              $product_name_org='<div class="wfm_name"><a href="'.WFM_BASE_URL.'/includes/wfm-popup-data.php?pid='.$val->ID.'" class="simple-ajax-popup-align-top">'.$val->post_title.'</a></div>';
              $prod_des='';
              if($val->post_content){
                $prod_des=$val->post_content;
                if(strlen($prod_des)>=120){
                  $prod_des = substr($prod_des,0,140).'... <a href="'.WFM_BASE_URL.'/includes/wfm-popup-data.php?pid='.$val->ID.'" class="simple-ajax-popup-align-top">Read More..</a>';
                }   
              }
              ...
              foreach($vq as $var_data){
                $product = wc_get_product($var_data->ID);                
              ...
              echo '<tr>';
              if (get_option('wfm_display_image')=='1'){
              ...
              if($chk=='on'){
                  ...
                  $cf_data= '<div style="display:none; width:100%;" class="wcf_add_cost wcf_add_cost_'.$val->ID.'">Additional cost: '.$curr.'<span id="wcf_c_'.$val->ID.'"></span></div>';
              ...
              echo '<td class="wfm_td_title">'.$product_name_org.'<div class="wfm_des">'.$prod_des.'</div></td><td>'.$cf_data.' '.$cf.'</td>';
              ?>
                    <td valign="top" class="wfm_td_quantity">
...
function wfm_show_prod($id, $wfm_img_size, $post_title){
  //wrm_item_custom_field($id);
    ...
    $product_name='<div class="wfm_name"><a href="'. WFM_BASE_URL.'/includes/wfm-popup-data.php?pid='.$id.'" class="simple-ajax-popup-align-top">'.$post_title.'</a>
      </div>';
    $product = wc_get_product($id);
    $prod_des='';
    if($product->post->post_content){
      ...
      if(strlen($prod_des)>=120){
         $prod_des = substr($prod_des,0,120).'... <a href="'. WFM_BASE_URL.'/includes/wfm-popup-data.php?pid='.$id.'" class="simple-ajax-popup-align-top">Read More..</a>';
      }
    }
    ...
    if (get_option('wfm_display_image')=='1'){
      if (get_option('wfm_display_image_preview')=='1'){
        echo '<td class="wfm_td_img"><a href="'.$img_url2.'" class="preview"><img src="'.$img_url.'" height="'.$wfm_img_size.'" width="'.$wfm_img_size.'" /></a></td>';
      }else{                
        echo '<td class="wfm_td_img"><img src="'.$img_url.'" height="'.$wfm_img_size.'" width="'.$wfm_img_size.'" /></td>';
      }
    }
    ...
    if($chk=='on'){
       echo '<td class="wfm_td_title">'.$product_name.'<div class="wfm_des">'.$prod_des.'</div></td>';
        $curr=get_woocommerce_currency_symbol();
        $cf_data= '<div style="display:none; width:100%;" class="wcf_add_cost wcf_add_cost_'.$id.'">Additional cost: '.$curr.'<span id="wcf_c_'.$id.'"></span></div>';
      ...
      echo '<td colspan="2" class="wfm_td_title">'.$product_name.'<div class="wfm_des">'.$prod_des.'</div></td>';
    }    
    ?>
      <td class="wfm_td_quantity" valign="top">
          <?php
          if($max_stock!=0){          
